...
- **背景**：用户第二次指出同类问题：开场白和对话仍按短句切得过碎，同一个人的连续发言
  ...
  仍使用 Whisper 的标点/停顿分段，完全没有说话人分离，说明规则没有覆盖所有访谈来源。
- **根因**：
  1. 合并阈值仅 3 秒/1000 字，正常思考停顿会错误断段。
  2. 前两分钟所有句子被强制标为主持人，嘉宾较早开口时身份错误。
  3. 只有 metadata 提供姓名才输出身份标签；没有姓名时退化成裸时间戳。
  4. 简短附和被无条件改成前一角色，覆盖了 cam++ 的真实说话人切换。
  5. YouTube 只有 Whisper segment；模型没有 diarization，不能可靠判断“谁在说话”。
  6. 多位嘉宾被统一映射为 `guest` 后，如果只比较 role，会错误合并不同嘉宾。
- **正确做法（强制）**：
  1. 有 `sentence_info` 时以说话人切换为主边界；同角色发言跨 ≤15 秒停顿继续合并。
  2. 开场白按 ≤1200 字/≤180 秒合并成少数大段；对话按 ≤1500 字/≤360 秒合并，
     角色切换立即断段。
  3. 仅以前 45 秒累计发言时长识别主要主持人，不再强制前两分钟所有声音为主持人。
  4. 保留 cam++ 对附和词的说话人判定，不把“嗯/对”等无条件并入前一角色。
  5. 有姓名显示姓名；无姓名也必须显示“主持人”/“嘉宾”。
  6. YouTube 采用混合流程：Whisper 保留文本质量，SenseVoiceSmall + CAM++ 只生成
     说话人时间区间，按时间重叠映射到 Whisper segment，再按同一 `spk` 合并。
  7. 合并必须优先比较 `spk`，只有缺失 `spk` 时才回退比较 role；不同嘉宾不得合并。
  8. CAM++ 不可用时禁止静默生成“看似有角色”的文字稿；必须报错，只有显式
     `--no-speakers` 才允许无角色转录。
- **自检项**：构造“主持人开场→嘉宾在两分钟内开口→短暂停顿继续长答→主持人追问”
  回归样例，必须得到角色正确的少数大段；匿名 metadata 仍有角色标签；另构造两位嘉宾
  连续发言样例，确保不同 `spk` 不合并。播客和 YouTube 两条链路都必须覆盖。
...
#### L041: Lenny's Newsletter 模版段未被抓取层过滤，且不能在双语阶段补删（2026-08-28，自检发现；2026-08-31 落地）
...
- **Community Wisdom 变体（2026-08-31）**：该栏目由 Kiyani 编辑，文末不是 Lenny 署名，而是
  `If anyone in your life would benefit from this newsletter…` 礼品订阅号召 +
  `[Give a gift subscription]` + `Kiyani 👋`。正文里的「gift」可能夹零宽空格，
  硬校验应同时覆盖链接文案 `give a gift subscription` 与段落标记
...
- **正确做法（已固化到 `fetch_article.py`）**：
  1. DOM 层：Substack 正文提取前剔除 Lenny 固定开场段（`👋 Hey there, I’m Lenny` /
     `👋 Hello and welcome to this week’s edition`）、`P.S. Get a full free year of`、
     `If you’re finding this newsletter valuable`、`If anyone in your life would benefit from this newsletter`、
     `Give a gift subscription` 链接，以及文末 `Sincerely,` / `Name 👋` 署名。
  2. 文本层兜底：`_strip_substack_archive_noise` 同步删除上述段落；噪音词表补
     `👋 hey there, i’m lenny`、`a full free year of`、`finding this newsletter valuable`、
     `would benefit from this newsletter`、`give a gift subscription`。
  3. 保留正文性质的结尾致谢（如 `*Thanks, Cliff! For more, check out Vocation.*`、
     `*Have a fulfilling and productive week*`），它们属于作者内容。
...
- **自检项**：Substack 工作包全文搜索 `👋 Hey there`、`a full free year of`、
  `finding this newsletter valuable`、`would benefit from this newsletter`、
...
- **问题**：双语 `> [!note]` 标题超过 80 字符后，uploader 把包含 `[!note]` 标记的整行
  ...
  但最终对账报“缺少长段落锚点”。该次新建失败后页面也未被自动删除。
- **根因**：上传器的锚点提取没有排除可移植 callout 指令行；异常回滚路径没有覆盖
  “内容全部写入但 verify_remote 失败”这一阶段。
- **正确做法**：
  1. Callout 英文标题行控制在 80 字符以内，中文标题放到紧随的同一 callout 子行；
     保持连续 `>`，避免标注被普通空行截断。
  2. 上传失败后立即重新列出目标目录；若失败页仍存在，禁止再次新建，改用其
     `entry_id` 覆盖重试，确保目录最终只有一个业务页面。
  3. 公共 uploader 应在后续修复中排除 `[!note]`/`[!stat]`/`[!definition]` 指令行锚点，
     并让新建页面在远程对账失败时也进入回滚路径。
- **自检项**：dry-run 的 `local_callouts` 必须符合预期；上传失败后的目录快照不得出现
  重复条目；最终必须 `verified=true` 且 `local_callouts == remote_callouts`。

#### L031: Quarto 正文容器与表格配图必须端到端核验（2026-07-21，自检发现）
- **问题**：Hamel.dev 等 Quarto 站点没有 `article` 节点，正文位于
  `#quarto-document-content` / `main`；旧抓取器虽然侦察到正文和 7 张图，转 Markdown
  阶段却输出空正文。补上正文容器后，表格转换又只读取 `innerText`，导致表格首列的
  7 张链接图片全部丢失。
- **根因**：侦察、图片枚举和 Markdown 转换使用了不同的容器选择器；表格单元格没有
  递归调用节点转换，空文本图片链接也没有继续处理子节点。
- **正确做法**：
  1. 通用容器同时覆盖 `#quarto-document-content` 和 `main`，侦察与转换共用同一集合。
  2. 表格单元格递归调用 `processNode`；链接无文字时继续转换子节点，保留链接图片。
  3. 抓取成功不能只看 `content_length`，必须确认 `source.md` 有真实正文，且
     `meta.image_count == source.md 本地图片引用数 == images/ 文件数`。
  4. 若本地图片位于 Markdown 表格单元格，上传器按图片分段可能拆坏表格；发布前必须
     预览线上 clean 结果。当前安全降级是按原顺序改成“标题→图片→说明”连续结构。
- **已修复**：`fetch_article.py` 的 Quarto 容器、空文本图片链接和表格单元格递归转换。
...
#### L028: 双语发布标题不能使用机械后缀；正文不能重复页面标题（2026-07-21，用户反馈）
- **问题**：非中文文章被发布成 `Original Title 中英对照`，中文语义标题缺失；正文首个
  H1 又与在线文档名称重复，造成视觉冗余。
- **根因**：归档层把 `--name-suffix " 中英对照"` 当作双语命名策略，同时未区分
  `meta.title`（原文文件契约）、`meta.display_title`（发布展示名）和正文标题。
- **正确做法**：
  1. 非中文内容由 `trans-doc-to-md` 生成
     `meta.display_title = "Original Title / 中文标题"`。
  2. uploader 的 `--name` 直接使用 `display_title`，禁止追加“中英对照”“双语版”。
  3. 最终 Markdown 删除与 `display_title` 对应的首个文档级 H1，再上传或覆盖原页面。
  4. `meta.title` 与最终 Markdown 文件名仍保留原文标题，不因展示名改变。
- **自检**：页面名称含 ` / ` 和中文；正文首块不是同名 H1；去重仍按来源 URL 与原文标题。

#### L030: 上传中途失败会遗留临时空页面（2026-07-21，用户反馈）
- **问题**：uploader 已创建在线页面后，首段写入、图片上传或线上对账失败，知识库中会
  留下空白或半成品临时页面。
- **根因**：上传流程只有“创建→写入→验证”，异常路径没有事务式回滚。
- **正确做法**：新建页面后任一阶段失败，公共 uploader 自动调用
  `entry_delete_entry` 将本次页面移入回收站；覆盖已有页面失败时绝不能删除原条目。
  清理失败必须同时报告原错误和临时条目 ID，禁止静默忽略。
- **补充根因**：即使 uploader 能回滚单次失败，Agent 若为诊断反复创建 `upload test`、
  `v2`、`中英对照` 等页面，仍会留下多个成功创建但无业务价值的临时条目。
- **补充做法**：归档编排在上传前记录目录子条目快照；诊断只用 `--dry-run`。交付前将
  上传后条目与快照对账，本次任务最多保留最终页面和 VOD 各一条；发现 test/v2/空白/
  重复条目必须停止交付并清理，禁止把清理工作转嫁给用户。
- **自检**：公共 uploader 回归测试分别覆盖“新建失败会删除”和“覆盖失败不删除”；
  归档层额外执行上传前后目录差异对账。
